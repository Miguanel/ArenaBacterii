...
// Funkcja idealnie zaginająca mapę (Torus World)
...
window.draw = function() {
    ...
    // Bezpieczne zapętlenie kamery, by minimapa nie wariowała
    window.cameraX = ((window.cameraX % window.WORLD_WIDTH) + window.WORLD_WIDTH) % window.WORLD_WIDTH;
    window.cameraY = ((window.cameraY % window.WORLD_HEIGHT) + window.WORLD_HEIGHT) % window.WORLD_HEIGHT;

    ctx.save();
    // ❌ USUNIĘTO BŁĘDNE ctx.translate() TUTAJ ❌

    // 🟢 NOWA, PRECYZYJNA SIATKA ZAWSZE ZGODNA Z MAPĄ
    ctx.strokeStyle = "#16ff0011"; ctx.lineWidth = 1;
    let startX = (canvas.width / 2) - window.cameraX;
    let startY = (canvas.height / 2) - window.cameraY;

    for (let x = (startX % 200) - 200; x < canvas.width; x += 200) {
        ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, canvas.height); ctx.stroke();
    }
    for (let y = (startY % 200) - 200; y < canvas.height; y += 200) {
